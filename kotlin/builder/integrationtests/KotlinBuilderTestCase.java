...
import io.bazel.kotlin.model.KotlinModel.CompilationTaskInfo;
import io.bazel.kotlin.model.KotlinModel.Platform;
import io.bazel.kotlin.model.KotlinModel.RuleKind;
...
abstract class KotlinBuilderTestCase {
  ...
  private final KotlinModel.JvmCompilationTask.Builder builder =
      KotlinModel.JvmCompilationTask.newBuilder();
  ...
  protected KotlinModel.JvmCompilationTask.Outputs outputs() {
    return builder.getOutputs();
  }

  protected KotlinModel.JvmCompilationTask.Directories directories() {
  ...
  protected KotlinModel.JvmCompilationTask builderCommand() {
  ...
  protected enum DirectoryType {
    ...
    protected static Path select(DirectoryType type, KotlinModel.JvmCompilationTask command) {
      Path ret;
      switch (type) {
        case CLASSES:
